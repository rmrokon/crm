...
function ClosedWon() {
    ...
    if (deals.deals.data) {
        dealsInClosedWon = deals.deals.data.filter(deal => deal.stage === 'Closed Won');
    }
    ...
    const addDealToBoard = (id) => {
        ...
        const newDeal = { stage: 'Closed Won', ...rest };
        ...
        <div>
            <header>
                <div className={styles.stageName}>
                    <span>Closed Won . 10%</span>
                    <h5>$0 . 0 Deals</h5>
                </div>
            </header>
            <main className={styles.dealsOnStage} ref={drop}>
                {
                    dealsInClosedWon?.map((deal) => <DraggableDeal key={deal._id} deal={deal} />)
                }
            </main>
        </div>
    )
